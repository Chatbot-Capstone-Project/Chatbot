...
@asset
def download_pdfs(context, webscrap):
    pdf_urls = webscrap[0]
    for url in pdf_urls:
        try:
...
def convert_php_to_pdf(context, webscrap):
    ...
    for i, url in enumerate(php_urls):
        ...
        pdf = FPDF()
        pdf.add_page()
        # Use a relative path for your font file
...
def collect_data(context, download_pdfs, convert_php_to_pdf):
    folder_path = 'PDFs'  # PDFs folder is at the root of the project folder
...
def store_in_chroma_db(context, collect_data):
    docs = collect_data
    db_path = 'chromaDB_client'  # the path to your ChromaDB client
